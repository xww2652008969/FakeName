        ...
        private void NewCustomCharaButton(Vector2 size)
        {
            ...
            if (Worlds != null)
            {
                if (ImGui.BeginPopup("AddCustomCharaContext", ImGuiWindowFlags.AlwaysAutoResize))
                {
                    ...
                    try
                    {
                        var worldRow = Worlds.First(w => w.IsPublic && !w.Name.Data.IsEmpty && w.Region == 2 && w.RowId == customWorld);
                        if (ImGui.BeginCombo("##指定角色服务器",  worldRow.Name.ToString(), ImGuiComboFlags.HeightLarge))
                        {
                            foreach (var world in Worlds.Where(w => w.IsPublic && !w.Name.Data.IsEmpty && w.Region == 2))
                            {
                    ...
                    {
                        if (ImGui.BeginCombo("##指定角色服务器",  "请选择服务器", ImGuiComboFlags.HeightLarge))
                        {
                            foreach (var world in Worlds.Where(w => w.IsPublic && !w.Name.Data.IsEmpty && w.Region == 2))
                            {
                                if (ImGui.Selectable($"{world.Name} ({world.RowId})",
