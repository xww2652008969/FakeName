...
public class TargetInfoComponent : IDisposable
{
    ...
    private unsafe void TargetInfoUpdate(AddonEvent type, AddonArgs args)
    {
        ...
        if (addon->IsVisible) RefreshTargetInfo(addon);
    }

    private unsafe void TargetInfoMainTargetUpdate(AddonEvent type, AddonArgs args)
    {
        var addon = (AtkUnitBase*)args.Addon;
        if (addon->IsVisible) RefreshIndependentTargetInfo(addon);
    }
    ...
    private unsafe void TargetTargetUpdate(AddonEvent type, AddonArgs args)
    {
        ...
        if (addon->IsVisible)
        {
            ...
            if (resNode != null && resNode->IsVisible()) RefreshTargetTarget((AtkUnitBase*)args.Addon);
        }
    }
    ...
    {
        var addon = (AtkUnitBase*)args.Addon;
        if (addon->IsVisible) RefreshFocusTargetInfo(addon);
    }

    private unsafe void WideTextUpdate(AddonEvent type, AddonArgs args)
    {
        var addon = (AtkUnitBase*)args.Addon;
        if (addon->IsVisible) RefreshWideText(addon);
    }
    ...
    private unsafe bool RefreshTargetInfo(AtkUnitBase* addon)
    {
        ...
        if (targetObj is not IPlayerCharacter targetChar) return false;

        if (!P.TryGetConfig(targetChar.Name.TextValue, targetChar.HomeWorld.RowId, out var characterConfig))
            return false;

        var textNode = addon->GetTextNodeById(16);
        var text = textNode->NodeText.ToString();

        var oriName = targetChar.Name.TextValue;
        var oriFcName = $"«{targetChar.CompanyTag.TextValue}»";
        var newName = characterConfig.FakeNameText.Length > 0
                          ? characterConfig.FakeNameText
                          : targetChar.Name.TextValue;
        var newFcName = characterConfig.FakeFcNameText.Length > 0
                            ? $"«{characterConfig.FakeFcNameText}»"
                            : $"«{targetChar.CompanyTag.TextValue}»";

        if (!(newName.Contains(oriName) && text.Contains(newName))) text = text.Replace(oriName, newName);
        if (!(newFcName.Contains(oriFcName) && text.Contains(newFcName))) text = text.Replace(oriFcName, newFcName);
    ...
    private unsafe bool RefreshIndependentTargetInfo(AtkUnitBase* addon)
    {
        ...
        var textNode = addon->GetTextNodeById(10);
        var text = textNode->NodeText.ToString();
    ...
    private unsafe bool RefreshTargetTarget(AtkUnitBase* addon)
    {
        ...
        var textNode = addon->GetTextNodeById(7);
        var text = textNode->NodeText.ToString();
        ...
        if (targetTargetObj != null)
        {
            ...
            else if (text.Contains(localPlayer.Name.TextValue)) targetTargetChara = localPlayer;
        }
        else
            targetTargetChara = localPlayer;

        if (targetTargetChara == null) return false;

        if (!P.TryGetConfig(targetTargetChara.Name.TextValue, targetTargetChara.HomeWorld.RowId,
                            out var characterConfig)) return false;

        var oriName = targetTargetChara.Name.TextValue;
        var newName = characterConfig.FakeNameText.Length > 0
                          ? characterConfig.FakeNameText
                          : targetTargetChara.Name.TextValue;
    ...
    private unsafe bool RefreshFocusTargetInfo(AtkUnitBase* addon)
    {
        ...
        if (focusTarget is not IPlayerCharacter targetChar) return false;
    ...
    {
        if (!P.TryGetConfig(name, worldId, out var characterConfig)) return false;

        var textNode = wideTextAddon->GetTextNodeById(3);
        var text = textNode->NodeText.ToString();
